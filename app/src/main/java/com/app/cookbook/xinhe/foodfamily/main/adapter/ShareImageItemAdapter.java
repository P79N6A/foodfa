...
public class ShareImageItemAdapter extends RecyclerView.Adapter<ShareImageItemAdapter.Holder> {

    private List<ShareImageItem> shareImageItems;
    private Context context;
    private LayoutInflater inflater;

    public ShareImageItemAdapter(Context context) {
        this.context = context;
        this.inflater = LayoutInflater.from(context);
    }

    public void setShareImageItems(List<ShareImageItem> shareImageItems) {
    ...
    public void onBindViewHolder(@NonNull final Holder holder, int position) {
        ShareImageItem imageItem = shareImageItems.get(position);
        if (position == 0) {
            LinearLayout.LayoutParams layoutParams = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT);
            layoutParams.setMargins(75, 30, 0, 30);
            holder.lin_layout.setLayoutParams(layoutParams);
        } else if (position == shareImageItems.size() - 1) {
            LinearLayout.LayoutParams layoutParams = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT);
            layoutParams.setMargins(101, 30, 90, 30);
            holder.lin_layout.setLayoutParams(layoutParams);
        } else {
            LinearLayout.LayoutParams layoutParams = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT);
            layoutParams.setMargins(101, 30, 0, 30);
            holder.lin_layout.setLayoutParams(layoutParams);
        }

        Glide.with(context).load(imageItem.getShareImage())
                .error(R.drawable.touxiang)
                .into(holder.share_image);

        holder.share_name.setText(imageItem.getShareNamr());
        //条目点击时间
