...
//                toggleText();
    ...
    public void formatCollapseText(CharSequence text) {
        ...
        if (line <= mLines) {
            ...
            // 最后一行的开始字符位置
            int lastLineStartIndex = layout.getLineStart(line - a);
            // 最后一行的结束字符位置
            int lastLineEndIndex = layout.getLineVisibleEnd(line - a);
            // 计算后缀的宽度
            ...
            // 获取最后一行的宽
            float lastLineWidth = layout.getLineWidth(line - a);
            // 如果大于屏幕宽度则需要减去部分字符
            ...
            spannable.append("...");
//            Log.e("手机端开发了",ellipsizeText.toString());
//            Log.e("手机端开发了",appd.toString());
            // 设置样式
