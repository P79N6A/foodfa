 ...
 * version: 1.0.2
 */
public class NormalExpandableTextView extends LinearLayout {
    private String TAG = this.getClass().getSimpleName();
    private final static String imgTag = "[img]";
    private String TIP_COLLAPSE = "内容收起";
    private String TIP_EXPAND = "展开描述";
    ...
    public NormalExpandableTextView(Context context) {
        this(context, null, 0);
    }

    public NormalExpandableTextView(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public NormalExpandableTextView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        if (typedArray != null) {
            ...
            mContentTextSize = typedArray.getDimensionPixelSize(R.styleable.ExpandableTextView_android_textSize, sp2px(context, 16));
        ...
        mTvExpand.setCompoundDrawablePadding(4);
        if (mExpandDrawable != null) {
...
//                mCancelAnim = false;
//                toggleText();
                editTextCallBack.onclick();

            }
        };

        mTvContent.setOnClickListener(clickListener);//去掉文字不可点击
    ...
    //设置回调函数
    public void setOnclickCallBack(EditTextCallBack meditTextCallBack) {
    ...
    public void toggleText() {
        ...
        //计算控件高度
        int expandHeight = getTextViewHeight(mTvContent) + (mPosition == ALIGN_RIGHT ? 2 : 0)+5;//稍微修正一下高度
        collapseHeight = getTextViewHeight(mTvContentTemp)+5;
        Log.d(TAG, "展开高度" + expandHeight);
        Log.d(TAG, "收起高度" + collapseHeight);
        //此处使用LinearLayout.LayoutParams控制辅助文本控件mTvContentTemp的高度
        final LayoutParams layoutParams = (LayoutParams) mTvContentTemp.getLayoutParams();
        if (expandHeight <= collapseHeight) {
            //说明无需折叠
            layoutParams.height = 0;
            mTvContentTemp.setLayoutParams(layoutParams);
            Log.e("数据库费了解到是",expandHeight+"");
        ...
        if (mIsExpand) {
            if (mCancelAnim) {
                Log.e("111数据库费了解到是",expandHeight+"");
                mTvContent.setHeight(expandHeight);
                layoutParams.height = 0;
                mTvContentTemp.setLayoutParams(layoutParams);
            } else {
                ...
                anim.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                    ...
                    public void onAnimationUpdate(ValueAnimator animation) {
                        Log.e("222数据库费了解到是",(Integer) animation.getAnimatedValue()+"");
    ...
    public void formatCollapseText(CharSequence text) {
        ...
        if (line <= mLines) {
            Log.e("数据库法律是",mTvContentTemp.getHeight()+"");
            ...
            spannable.append("...");
//            Log.e("111手机端开发了",ellipsizeText.toString());
//            Log.e("111手机端开发了",appd.toString());
            // 设置样式
