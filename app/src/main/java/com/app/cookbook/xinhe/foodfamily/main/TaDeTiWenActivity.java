...
import com.app.cookbook.xinhe.foodfamily.main.adapter.MyTiWenAdapter;
...
public class TaDeTiWenActivity extends BaseActivity {
    ...
    TextView title_two;
    MyTiWenAdapter tiWenAdapter;
    String id;
    private int PAGE = 1;
    private boolean is_not_more;
    LinearLayoutManager layoutManager;
    List<TiWenEntity> dates = new ArrayList<>();
    ...
    public void initParms(Bundle parms) {
        ...
        other_user_id = parms.getString("other_user_id");
        Log.e("123", "      userId   " + id);
    }

    @Override
    public void initView() {
        setContentLayout(R.layout.activity_ta_de_ti_wen);
        //初始化黄牛刀
    ...
    private void getItemClick() {
        tiWenAdapter.setOnItemClickListener(new MyTiWenAdapter.OnItemClickListener() {
            @Override
            public void onItemClick(View view, int position) {
                TiWenEntity item = dates.get(position);
                Intent intent = new Intent(TaDeTiWenActivity.this, FenLeiQuestionDetailActivity.class);
    ...
    private void init_net_source(boolean is_show_dialog) {
        ...
        Log.e("获取ta提问参数：", params.toString());
        subscription = Network.getInstance("获取ta提问", this)
                .get_ta_question(params,
                        new ProgressSubscriberNew<>(TiWenDate.class, new GsonSubscriberOnNextListener<TiWenDate>() {
                            ...
                            public void on_post_entity(TiWenDate result) {
                                Log.e("得到的list的长度", "" + result.getData().size());
                                if (dates.size() > 0 && PAGE > 1) {//这表示是"加载"
                                    if (result.getData().size() == 0) {
                                        //表示没有更多数据了
                                        is_not_more = true;
                                        dates.addAll(result.getData());
                                        tiWenAdapter.notifyDataSetChanged();
                                    } else {
                                        is_not_more = false;
                                        dates.addAll(result.getData());
                                        tiWenAdapter.notifyDataSetChanged();
    ...
    private void set_list_resource(final List<TiWenEntity> dates) {
        ...
        tiWenAdapter = new MyTiWenAdapter(dates, getApplicationContext(), ista);
        tiWenAdapter.setTitleName("TA的提问");
        recycler_view.setAdapter(tiWenAdapter);
        getItemClick();
        ...
        if (dates.size() > 7) {
            xrefreshview.enableReleaseToLoadMore(true);
//            tiWenAdapter.setCustomLoadMoreView(new XRefreshViewFooter(this));//加载更多
