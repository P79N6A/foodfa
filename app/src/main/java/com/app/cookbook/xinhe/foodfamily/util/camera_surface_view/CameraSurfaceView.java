...
public class CameraSurfaceView extends SurfaceView implements SurfaceHolder.Callback, Camera.AutoFocusCallback, View.OnClickListener {
    ...
    Context context;
    private int normal_width=1080;
    private int normal_height=1080;
...
//            Camera.Size previewSize = CamParaUtil.getSize(mParam.getSupportedPreviewSizes(), 1080,
//                    mCamera.new Size(VIDEO_720[0], VIDEO_720[1]));
            Camera.Size previewSize = getCloselyPreSize(true,normal_width,normal_height,mParam.getSupportedPreviewSizes());
            mParam.setPreviewSize(previewSize.width, previewSize.height);

            Log.e("哈哈尺寸previewSize","width:"+previewSize.width+"height:"+previewSize.height);
            ...
            Camera.Size pictureSize = CamParaUtil.getSize(mParam.getSupportedPictureSizes(), 1080,
                    mCamera.new Size(previewSize.width, previewSize.height));

            Log.e("哈哈相机pictureSize","width:"+pictureSize.width+"height:"+pictureSize.height);
...
//        startPreview();
    ...
    public void surfaceChanged(SurfaceHolder holder, int format, int mwidth, int mheight) {
        Log.e("加速快放假", "surfaceChanged: " + mwidth + "  " + mheight);
        Log.e("111谁先进来的","谁先进来的");
        normal_height=mheight;
        normal_width = mwidth;
        stopPreview();
        startPreview();
    }
     ...
     **/
    CameraBack mcameraBack;
    public void capture(CameraBack cameraBack) {
            ...
            try {
                mCamera.takePicture(null, null, new Camera.PictureCallback() {
                    ...
                    public void onPictureTaken(byte[] data, Camera camera) {
                        ...
                        String return_path=FileUtil.saveBitmap(bitmap);
                        startPreview();
                        mcameraBack.return_picture_path(return_path);
                    }
                });
            } catch (Exception e) {
                if (isRecording) {
